...
import { Search, MapPin, Briefcase, SlidersHorizontal, RefreshCcw, X } from "lucide-react";
  ...
  const filteredJobs = useMemo(() => {
    return jobs.filter((job) => {
      ...
      if (keyword) {
        ...
        const matchesSkills = job.skills.some((s) => s.toLowerCase().includes(keyword));
      ...
      if (filters.companyTypes.length > 0 && !filters.companyTypes.includes(job.companyType)) {
        return false;
      }
      ...
      if (filters.locations.length > 0) {
        ...
        const matchesSelectedLoc = filters.locations.some((loc) => jobCity.includes(loc));
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
        ...
        <div className="bg-white border border-border shadow-sm rounded-2xl p-4">
          <form onSubmit={handleTopSearchSubmit} className="grid grid-cols-1 md:grid-cols-12 gap-3 items-center">
            {/* Input keywords */}
          ...
          <div className="col-span-12 md:col-span-8 lg:col-span-9 space-y-4">
            ...
            <div className="hidden md:flex items-center justify-between border-b border-slate-100 pb-3">
              ...
              <div className="text-xs text-muted-foreground">
                Sort by: <span className="font-bold text-slate-700">Relevance</span>
              </div>
            </div>
            ...
            {filteredJobs.length > 0 ? (
              ...
              <div className="bg-white border border-border rounded-3xl p-12 text-center max-w-lg mx-auto space-y-4 shadow-sm">
                ...
                <p className="text-sm text-muted-foreground leading-relaxed">
                  We couldn't find any job openings matching your current selection. Try broadening your keywords, choosing different locations, or resetting your filter sidebar.
                </p>
                <button
                  onClick={handleReset}
    ...
    <Suspense fallback={
      <div className="flex h-screen w-full items-center justify-center bg-slate-50 text-slate-600">
        <div className="flex flex-col items-center gap-2">
          ...
          <span className="text-sm font-semibold text-slate-500">Loading Job Listings...</span>
        </div>
      </div>
    }>
      <JobsListContent />
