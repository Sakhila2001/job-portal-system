    ...
    const fetchLookups = async () => {
      try {
        ...
        const [deptRes, desigRes] = await Promise.all([
          fetch(`${API_BASE_URL}/api/admin/departments?limit=100`, {
            headers,
            credentials: "include",
          }),
          fetch(`${API_BASE_URL}/api/admin/designations?limit=100`, {
            headers,
            credentials: "include",
          }),
        ]);
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      const response = await fetch(
        isEditing ? `${API_BASE_URL}/api/recruiter/jobs/${job!.id}` : `${API_BASE_URL}/api/recruiter/jobs`,
        {
