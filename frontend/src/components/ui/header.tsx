...
import CandidateLoginDrawer from "@/features/auth/components/candidate-login-drawer";
import EmployerAuthDrawer from "@/features/auth/components/employer-auth-drawer";
...
export default function Header() {
  ...
  const handleResendEmployerVerification = async () => {
    ...
    const API_BASE_URL_RESEND = process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000";

    try {
      const response = await fetch(`${API_BASE_URL_RESEND}/api/auth/resend-verification`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
