...
import {
  ...
  hashPassword,
} from "../repositories/register.repository";
import { sendEmail } from "../email";
import prisma from "../prisma";
import { createAccessToken, createRefreshSession } from "../session";
...
export async function registerEmployer(data: EmployerRegisterDto) {
  ...
  let baseSlug = parsed.companyName
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "");
  ...
  await createNotification(user.id, "email", "verification", "Verify your email - JobPortal", { email: user.email });
  sendVerificationEmail(user.email, parsed.companyName, token).catch((err) => console.error("[email] employer verification failed:", err));
  sendWelcomeEmail(user.email, parsed.companyName).catch((err) => console.error("[email] employer welcome failed:", err));
