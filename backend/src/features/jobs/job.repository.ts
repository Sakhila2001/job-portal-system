...
export class JobRepository {
  ...
  static async syncJobResponsibilities(
    jobId: string,
    responsibilities: (string | { responsibility: string; displayOrder?: number; isKey?: boolean })[]
  ) {
    await prisma.jobResponsibility.deleteMany({ where: { jobId } });

    let order = 0;
    for (const item of responsibilities) {
      const text = typeof item === "string" ? item.trim() : item.responsibility.trim();
      const displayOrder = typeof item === "object" ? item.displayOrder ?? order : order;
      const isKey = typeof item === "object" ? item.isKey ?? true : true;
  ...
  static async syncJobQualifications(
    jobId: string,
    qualifications: (string | { name: string; level?: string | undefined; isRequired?: boolean | undefined; isPreferred?: boolean | undefined })[]
  ) {
    ...
    for (const item of qualifications) {
      ...
      const isPreferred = typeof item === "object" ? item.isPreferred ?? (!isRequired) : false;
